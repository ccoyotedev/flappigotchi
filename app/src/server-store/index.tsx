...
import { AavegotchiObject, HighScore } from 'types';
...
export const ServerProvider = ({
  ...
  const snapshotListener = (database: fb.firestore.Firestore, gotchis: Array<AavegotchiObject>) => {
    return database.collection(process.env.REACT_APP_COLLECTION_NAME || "")
      .withConverter(converter)
      .where("tokenId", "in", gotchis.map(gotchi => gotchi.id))
      .onSnapshot((snapshot) => {
        snapshot.docChanges().forEach((change) => {
          const changedItem = change.doc.data();
          const newHighscores = myHighscoresRef.current ? [...myHighscoresRef.current] : [];
  ...
  useEffect(() => {
    if (usersGotchis && usersGotchis.length > 0 && firebase && initiated) {
      ...
      const listenerArray = gotchiSetArray.map(gotchiArray => snapshotListener(db, gotchiArray));
